...
class Song extends Model
{
    ...
    */
        ...
        WHERE sa.song_id = ?
          AND NOT EXISTS (
              ...
              WHERE h.user_id = ?
                ...
                AND h.played_at >= DATE_TRUNC('month', CURRENT_DATE)
          )
    ");
    ...
    public function cardsByIds(array $ids): array
    {
        ...
        $stmt = $this->db->prepare("
            SELECT
                ...
                STRING_AGG(a.id::text || '::' || a.name || '::' || a.slug, '||' ORDER BY CASE WHEN sa.role = 'primary' THEN 0 ELSE 1 END, a.name ASC) AS artist_data
    ...
    private function bindDynamic(
        \App\Core\PostgresStatement $stmt,
        array $params
    ): void {
        $refs = [];
